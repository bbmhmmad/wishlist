...
function handleRender(req,res){
	...
	.then(function(response){
		...
		//STEP1 CREATE REDUX STORE ON SERVER
		const store = createStore(reducers,{'books':{'books':response.data}})
		//STEP2 GET INITIAL STATE FROM STORE
		const initialState = JSON.stringify(store.getState());
		//STEP3 IMPLEMENT REACT-ROUTER ON THE SERVER TO INTERCEPT CLIENT REQUESTS AND DEFINE WHAT TO DO WITH THEM
		const Routes = {routes:routes,location:req.url}
		match(Routes,function(error,redirect,props){
			if(error){
				res.status(500).send('Error fulfilling request')
			}
			else if(redirect){
